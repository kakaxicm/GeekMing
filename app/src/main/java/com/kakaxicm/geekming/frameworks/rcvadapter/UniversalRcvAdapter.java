package com.kakaxicm.geekming.frameworks.rcvadapter;
 ...
 */
public class UniversalRcvAdapter<T> extends RecyclerView.Adapter<UniversalRcvViewHolder>{
    private List<T> mData;

    public UniversalRcvAdapter(List<T> data , OnBindDataInterface<T> bindInterface){
        mData = data;
        mOnBindDataInterface = bindInterface;
    }

    public UniversalRcvAdapter(List<T> data , OnMultiTypeBindDataInterface<T> bindInterface){
        mData = data;
    ...
    public interface OnBindDataInterface<T>{
        void onBindData(T model, UniversalRcvViewHolder holder, int pos, int type);
    ...
    @Override
    public UniversalRcvViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //构建ViewHolder 入参 parant viewType layoutid
        int layoutId = mOnBindDataInterface.getItemLayoutId(viewType);
        UniversalRcvViewHolder holder = UniversalRcvViewHolder.getViewHolder(parent, layoutId);
        return holder;
    }

    @Override
    public void onBindViewHolder(UniversalRcvViewHolder holder, int position) {
